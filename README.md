...
1. **`SmplGarmentManager`**: keep **Drive Garment Armature From SMPL** off (remap mode). If it is on and bone names do not match, the drive map can be empty — the script now **falls back to remap** and logs a warning. With drive off, leave **Recalculate Bind Poses After Remap** on. If sleeves still stretch, try **Bind Pose Reference → Root Bone World** on the component (mesh-space vs root-bone-space for inverse bind matrices).
