...
1. **`SmplGarmentManager`**: keep **Drive Garment Armature From SMPL** off (remap mode). Leave **Recalculate Bind Poses After Remap** on. If sleeves still stretch, try **Bind Pose Reference → Root Bone World** on the component (mesh-space vs root-bone-space for inverse bind matrices).
