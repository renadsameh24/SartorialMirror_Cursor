...
- **From two FBXs (SMPL + raw shirt), no .blend file:** run `./Tools/run_blender_golden_garment_from_fbx.sh /abs/path/shirt.fbx` or set `SMPL_FBX`, `GARMENT_FBX`, `EXPORT_FBX` and run `Tools/blender_golden_garment_from_fbx.py` in Blender (see script header). The script **auto-scales the garment to SMPL** before weights (default `AUTO_SCALE_MODE=combined`: bbox diagonal, max AABB side, torso `J00→J12`/`J15`, and shoulder `J16→J17` vs garment span — median of available metrics; clamps `AUTO_SCALE_S_MIN`/`AUTO_SCALE_S_MAX`). Then it pre-creates **vertex groups**, runs **Data Transfer** (`POLYINTERP_NEAREST` first), optionally **KD-tree nearest-vertex** weight copy (`FORCE_KD=1`, default in the shell helper), **normalizes**, **limits to 4 influences** (Unity), then exports. Open Unity and let the prepared FBX reimport (match SMPL **Scale Factor** on the importer, usually **100**). If Unity still reports “only 2 bones,” the catalog prefab is probably still pointing at an **old** FBX — link `Assets/garments_prepared/Flannel_SMPL_Skinned.fbx` in **GarmentCatalog** and reimport.
- **Scale only in Blender (weights already good):** `./Tools/run_blender_scale_match_to_unity.sh` imports the **single** prepared FBX (`GARMENT_FBX`, default `Assets/garments_prepared/Flannel_SMPL_Skinned.fbx`), runs **AUTO_SCALE** (`AUTO_SCALE_MODE=combined` by default), then exports to `EXPORT_FBX` **only if** a safe scale was applied. Shirt-only FBXs (no body mesh) use **armature vs shirt bbox** heuristics; factors outside **0.2–5×** are **skipped** unless you set `ALLOW_EXTREME_ARMATURE_SCALE=1` (avoids destroying an already Unity-sized asset). For the most reliable Blender sizing with a **body reference**, use the full two-FBX script `./Tools/run_blender_golden_garment_from_fbx.sh` instead. Under the hood: `SCALE_MATCH_ONLY=1` in `Tools/blender_golden_garment_from_fbx.py`.
...
### Garment still stretches or arms look wrong in Play Mode

1. **`SmplGarmentManager`**: keep **Drive Garment Armature From SMPL** off (remap mode). If it is on and bone names do not match, the drive map can be empty — the script **falls back to remap** and logs a warning. **Recalculate Bind Poses After Remap** defaults to **off** so the shirt only repoints bones to scene SMPL (usually enough to follow the pose). Turn **Recalculate Bind Poses** **on** only if you see stretched sleeves / wrong arm length (and enable **Read/Write** on the garment FBX import if needed). If sleeves still stretch with recalc on, try **Bind Pose Reference → Root Bone World**.
2. **Optional weight fix**: enable **Decouple Head From Arm Weights** only if logs show heavy **J15 + arm** mix on the same mesh.
...
5. **Shirt frozen but looks fine:** Duplicate `J00`…`J23` names can come from the garment under `_Garments` **or** a second armature next to the live rig. The manager now prefers the subtree that `SpheresToBones_FKDriver` drives (via `rootBone`). If it still freezes, set **`Smpl Armature Root Override`** on `SmplGarmentManager` to the **driven** armature object (parent of pelvis/J00, **not** under `_Garments`). Check the Console for errors about pelvis ≠ FK `rootBone`.
